 ...
 *
 * Custom paths only:
 *   node scripts/make-prompt.mjs --paths README.md docs package.json
 *
 * Show included files only:
 *   node scripts/make-prompt.mjs --list-only
 *
 * Combined examples:
 *   node scripts/make-prompt.mjs --paths README.md docs --tree --diff
...
function isExcluded(filePath) {
    ...
    if (!withTests && base.includes('.spec.')) return true;
...
function main() {
    ...
    const uniq = Array.from(new Set(files)).sort((a, b) => rel(a).localeCompare(rel(b)));
